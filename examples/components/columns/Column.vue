<template>
  <vb-section>
    <example-section-item title="Basic">
      <demo>
        <vb-cols>
          <vb-col>
            <p class="bg bg-primary">First column</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-primary">Second column</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-primary">Third column</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-primary">Fourth column</p>
          </vb-col>
        </vb-cols>
      </demo>
    </example-section-item>

    <example-section-item title="Sizes">
      <demo>
        <vb-cols>
          <vb-col size='1'>
            <p class="bg bg-primary">1</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">1</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">1</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">1</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">1</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">1</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">1</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">1</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">1</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">1</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">1</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">1</p>
          </vb-col>
        </vb-cols>

        <vb-cols>
          <vb-col size='2'>
            <p class="bg bg-primary">2</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">1</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">1</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">1</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">1</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">1</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">1</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">1</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">1</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">1</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">1</p>
          </vb-col>
        </vb-cols>
        <vb-cols>
          <vb-col size='2'>
            <p class="bg bg-primary">2</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">1</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">1</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">1</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">1</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">1</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">1</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">1</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">1</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">1</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">1</p>
          </vb-col>
        </vb-cols>
        <vb-cols>
          <vb-col size='3'>
            <p class="bg bg-primary">3</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">1</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">1</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">1</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">1</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">1</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">1</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">1</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">1</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">1</p>
          </vb-col>
        </vb-cols>
        <vb-cols>
          <vb-col size='4'>
            <p class="bg bg-primary">4</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">1</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">1</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">1</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">1</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">1</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">1</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">1</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">1</p>
          </vb-col>
        </vb-cols>
        <vb-cols>
          <vb-col size='5'>
            <p class="bg bg-primary">5</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">1</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">1</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">1</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">1</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">1</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">1</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">1</p>
          </vb-col>
        </vb-cols>
        <vb-cols>
          <vb-col size='6'>
            <p class="bg bg-primary">6</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">1</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">1</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">1</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">1</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">1</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">1</p>
          </vb-col>
        </vb-cols>
        <vb-cols>
          <vb-col size='7'>
            <p class="bg bg-primary">7</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">1</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">1</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">1</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">1</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">1</p>
          </vb-col>
        </vb-cols>
        <vb-cols>
          <vb-col size='8'>
            <p class="bg bg-primary">8</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">1</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">1</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">1</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">1</p>
          </vb-col>
        </vb-cols>
        <vb-cols>
          <vb-col size='9'>
            <p class="bg bg-primary">9</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">1</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">1</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">1</p>
          </vb-col>
        </vb-cols>
        <vb-cols>
          <vb-col size='10'>
            <p class="bg bg-primary">10</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">1</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">1</p>
          </vb-col>
        </vb-cols>
        <vb-cols>
          <vb-col size='11'>
            <p class="bg bg-primary">11</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">1</p>
          </vb-col>
        </vb-cols>
        <vb-cols>
          <vb-col size='12'>
            <p class="bg bg-primary">12</p>
          </vb-col>
        </vb-cols>
        <vb-cols>
          <vb-col size='full'>
            <p class="bg bg-primary">full</p>
          </vb-col>
        </vb-cols>
        <vb-cols>
          <vb-col size='four-fifths'>
            <p class="bg bg-primary">four-fifths</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">Auto</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">Auto</p>
          </vb-col>
        </vb-cols>
        <vb-cols>
          <vb-col size='three-quarters'>
            <p class="bg bg-primary">three-quarters</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">Auto</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">Auto</p>
          </vb-col>
        </vb-cols>
        <vb-cols>
          <vb-col size='two-thirds'>
            <p class="bg bg-primary">two-thirds</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">Auto</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">Auto</p>
          </vb-col>
        </vb-cols>
        <vb-cols>
          <vb-col size='three-fifths'>
            <p class="bg bg-primary">three-fifths</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">Auto</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">Auto</p>
          </vb-col>
        </vb-cols>
        <vb-cols>
          <vb-col size='half'>
            <p class="bg bg-primary">half</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">Auto</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">Auto</p>
          </vb-col>
        </vb-cols>
        <vb-cols>
          <vb-col size='two-fifths'>
            <p class="bg bg-primary">two-fifths</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">Auto</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">Auto</p>
          </vb-col>
        </vb-cols>
        <vb-cols>
          <vb-col size='one-third'>
            <p class="bg bg-primary">one-third</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">Auto</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">Auto</p>
          </vb-col>
        </vb-cols>
        <vb-cols>
          <vb-col size='one-quarter'>
            <p class="bg bg-primary">one-quarter</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">Auto</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">Auto</p>
          </vb-col>
        </vb-cols>
        <vb-cols>
          <vb-col size='one-fifth'>
            <p class="bg bg-primary">one-fifth</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">Auto</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-light">Auto</p>
          </vb-col>
        </vb-cols>
      </demo>
    </example-section-item>

    <example-section-item title="Offset">
      <demo>
        <vb-cols>
          <vb-col size="half" offset="one-quarter">
            <p class="bg bg-primary">size:half<br />offset:one-quarter</p>
          </vb-col>
        </vb-cols>
        <vb-cols>
          <vb-col size="three-fifths" offset="one-fifth">
            <p class="bg bg-primary">size:three-fifths<br />offset:one-fifth</p>
          </vb-col>
        </vb-cols>
        <vb-cols>
          <vb-col size="4" offset="8">
            <p class="bg bg-primary">size:4<br />offset:8</p>
          </vb-col>
        </vb-cols>
        <vb-cols>
          <vb-col size="11" offset="1">
            <p class="bg bg-primary">size:11<br />offset:1</p>
          </vb-col>
        </vb-cols>
      </demo>
    </example-section-item>

    <example-section-item title="Narrow">
      <demo>
        <vb-cols>
          <vb-col narrow>
            <div style="width: 200px;">
              <p class="bg bg-primary">This column is only 200px wide</p>
            </div>
          </vb-col>
          <vb-col>
            <p class="bg bg-primary">This column will take up the remaining space available</p>
          </vb-col>
        </vb-cols>
      </demo>
    </example-section-item>

    <example-section-item title="Mobile columns">
      <demo>
        <vb-cols view="mobile">
          <vb-col>
            <p class="bg bg-primary">1</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-primary">2</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-primary">3</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-primary">4</p>
          </vb-col>
        </vb-cols>
      </demo>
    </example-section-item>

    <example-section-item title="Desktop columns">
      <demo>
        <vb-cols view="desktop">
          <vb-col>
            <p class="bg bg-primary">1</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-primary">2</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-primary">3</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-primary">4</p>
          </vb-col>
        </vb-cols>
      </demo>
    </example-section-item>

    <example-section-item title="Different column sizes per breakpoint">
      <demo>
        <vb-cols view="mobile">
          <vb-col mobile="three-quarters" tablet="two-thirds" desktop="half" widescreen="one-third" fullhd="one-quarter">
            <p class="bg bg-primary">mobile:three-quarters<br />
              tablet:two-thirds<br />
              desktop:half<br />
              widescreen:one-third<br />
              fullhd:one-quarter</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-primary">2</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-primary">3</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-primary">4</p>
          </vb-col>
        </vb-cols>
      </demo>
    </example-section-item>

    <example-section-item title="Nesting">
      <demo>
        <vb-cols>
          <vb-col>
            <p class="bg bg-primary">First column</p>
            <vb-cols>
              <vb-col>
                <p class="bg bg-primary">First nested column</p>
              </vb-col>
              <vb-col>
                <p class="bg bg-primary">Second nested column</p>
              </vb-col>
            </vb-cols>
          </vb-col>
        </vb-cols>
      </demo>
    </example-section-item>

    <example-section-item title="Gapless">
      <demo>
        <vb-cols gapType="gapless">
          <vb-col>
            <p class="bg bg-primary">First column</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-info">Second column</p>
          </vb-col>
        </vb-cols>
      </demo>
    </example-section-item>

    <example-section-item title="Variable gap">
      <demo>
        <vb-tags slot="control">
          <vb-tag class="cursor" v-for="(value,index) in values" :key="index" @mouseente="handleMouseente(value,$event)">{{value}}</vb-tag>
        </vb-tags>

        <vb-cols gapType="variable" :gap="gapValue">
          <vb-col v-for="(count,index) in counts" :key="index">
            <p class="bg bg-primary">{{count}}</p>
          </vb-col>
        </vb-cols>
      </demo>
    </example-section-item>

    <example-section-item title="Breakpoint based column gaps">
      <demo>
        <vb-cols gapType="variable" mobileGap="8" tabletGap="0" desktopGap="3" widescreenGap="8" fullhdGap="2">
          <vb-col v-for="(count,index) in counts" :key="index">
            <p class="bg bg-primary">{{count}}</p>
          </vb-col>
        </vb-cols>
      </demo>
    </example-section-item>

    <example-section-item title="Vertical alignment">
      <demo>
        <vb-cols align="vcentered">
          <vb-col size="8">
            <p class="bg bg-primary">1</p>
          </vb-col>
          <vb-col>
            <p class="bg bg-primary">2<br />Second column with more content. This is so you can see the vertical alignment.</p>
          </vb-col>
        </vb-cols>
      </demo>
    </example-section-item>

    <example-section-item title="Centering columns">
      <demo>
        <vb-cols align="centered">
          <vb-col size="8">
            <p class="bg bg-primary">8</p>
          </vb-col>
        </vb-cols>
        <vb-cols align="centered">
          <vb-col size="2">
            <p class="bg bg-primary">2</p>
          </vb-col>
          <vb-col size="2">
            <p class="bg bg-primary">2</p>
          </vb-col>
        </vb-cols>
      </demo>
    </example-section-item>

    <example-section-item title="Multiline">
      <demo>
        <vb-cols multiline>
          <vb-col size="8">
            <p class="bg bg-primary">8</p>
          </vb-col>
          <vb-col size="4">
            <p class="bg bg-primary">4</p>
          </vb-col>
          <vb-col size="6">
            <p class="bg bg-primary">6</p>
          </vb-col>
          <vb-col size="6">
            <p class="bg bg-primary">6</p>
          </vb-col>
        </vb-cols>
      </demo>
    </example-section-item>

    <example-section-item title="Multiline and Centering">
      <demo>
        <vb-cols view="mobile" align="centered" multiline>
          <vb-col narrow>
            <p class="bg bg-primary">First Column</p>
          </vb-col>
          <vb-col narrow>
            <p class="bg bg-primary">Our Second Column</p>
          </vb-col>
          <vb-col narrow>
            <p class="bg bg-primary">Third Column</p>
          </vb-col>
          <vb-col narrow>
            <p class="bg bg-primary">The Fourth Column</p>
          </vb-col>
          <vb-col narrow>
            <p class="bg bg-primary">Fifth Column</p>
          </vb-col>
        </vb-cols>
      </demo>
    </example-section-item>
  </vb-section>
</template>

<script>
export default {
  name: 'ColumnsExamples',
  data() {
    return {
      counts: ['1', '2', '3', '4', '5', '6'],
      values: ['0', '1', '2', '3', '4', '5', '6', '7', '8'],
      gapValue: null
    }
  },
  methods: {
    handleMouseente(value, event) {
      event.currentTarget.parentNode.childNodes.forEach(e => {
        e.classList.remove('bg-info')
      })
      event.currentTarget.classList.add('bg-info')
      this.gapValue = value
    }
  }
}
</script>

<style>
.bg {
  text-align: center;
  padding: 1rem;
  border-radius: 4px;
}
.bg-primary {
  background-color: #00d1b2;
  color: #fff;
}
.bg-info {
  background-color: #209cee !important;
  color: #fff !important;
}
.bg-light {
  background-color: #f5f5f5;
  color: #000;
}
.cursor {
  cursor: pointer;
  color: #fff;
}
</style>
